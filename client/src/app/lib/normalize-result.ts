import type { SavedResultArtifactDetail } from "./result-artifacts-api";
...
export function normalizeRemoteResult(detail: SavedResultArtifactDetail): ResultRecord {
  ...
  return {
    ...
    supportingContents: Array.isArray(snapshot.supportingContents)
      ? snapshot.supportingContents.filter(isRecord).map((content) => ({
          ...
          contentUrl: typeof content.contentUrl === "string" ? content.contentUrl : undefined,
          coverUrl: typeof content.coverUrl === "string" ? content.coverUrl : null,
    ...
    lowFollowerEvidence: Array.isArray(snapshot.lowFollowerEvidence)
      ? snapshot.lowFollowerEvidence.filter(isRecord).map((item) => ({
          ...
          playCount: asString(item.playCount, "互动未知"),
    ...
    aiTopicSuggestions: Array.isArray(snapshot.aiTopicSuggestions)
      ? (snapshot.aiTopicSuggestions as unknown[]).filter(isRecord).map((topic) => ({
          ...
          commentScore: typeof topic.commentScore === "number" ? topic.commentScore : undefined,
          commentReason: typeof topic.commentReason === "string" ? topic.commentReason : undefined,
          commentCount: typeof topic.commentCount === "number" ? topic.commentCount : undefined,
          supplyGapScore: typeof topic.supplyGapScore === "number" ? topic.supplyGapScore : undefined,
          supplyGapReason: typeof topic.supplyGapReason === "string" ? topic.supplyGapReason : undefined,
          lowFollowerScore: typeof topic.lowFollowerScore === "number" ? topic.lowFollowerScore : undefined,
          lowFollowerReason: typeof topic.lowFollowerReason === "string" ? topic.lowFollowerReason : undefined,
          lowFollowerSampleCount: typeof topic.lowFollowerSampleCount === "number" ? topic.lowFollowerSampleCount : undefined,
          evidenceContentIds: Array.isArray(topic.evidenceContentIds)
            ? (topic.evidenceContentIds as unknown[]).filter((id): id is string => typeof id === "string")
            : undefined,
        }))
      : undefined,
