 ...
 * ===============================
 * 爆款预测结果页 - 新 Figma 极简设计风格
 * 已改造为 Registry 渲染器，接受真实 ResultRecord 数据
 */
...
function getWindowStrengthLabel(w: string): string {
  const map: Record<string, string> = {
    strong_now: "稳定偏升",
    validate_first: "需验证",
    observe: "观察中",
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-[12px] text-[#6A7282] mt-0.5">机会分</span>
      </div>
    </div>
  );
}
...
function ContentVideoCard({ content }: { content: PredictionSupportingContent }) {
  ...
  return (
    <div className="rounded-[16px] border border-[#F3F4F6] overflow-hidden bg-white hover:shadow-md transition-shadow duration-200">
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 text-[11px] text-[#99A1AF] mb-2">
          ...
          {content.viewCount != null && <span>{formatNumber(content.viewCount)}播放</span>}
        </div>
        <button
...
/* ------------------------------------------------------------------ */
/*  行动按钮组 - 基于 bestActionNow + recommendedNextTasks              */
/* ------------------------------------------------------------------ */
function ActionButtons({ result }: { result: ResultRecord }) {
  const actions = [
    {
      icon: <Zap className="w-4 h-4" />,
      label: result.bestActionNow.ctaLabel,
      desc: result.bestActionNow.description,
      primary: true,
    },
    ...(result.recommendedNextTasks ?? []).slice(0, 2).map((task) => ({
        ...
        <button
          ...
          onClick={() => {
            window.dispatchEvent(new CustomEvent("open-deep-dive", {
              detail: { prompt: action.label },
            }));
          }}
          className={`flex flex-col items-start gap-2 p-4 rounded-[16px] border transition-all duration-200 text-left ${
            action.primary
              ? "border-[rgba(137,121,255,0.4)] bg-[#F9F8FF] hover:bg-[#F0EEFF]"
              : "border-[#F3F4F6] bg-[#F9FAFB] hover:bg-[#F3F4F6]"
          }`}
        >
          <div className={`w-8 h-8 rounded-[10px] flex items-center justify-center ${
            action.primary ? "bg-[#8979FF] text-white" : "bg-white text-[#6A7282] border border-[#E9EAEC]"
          }`}>
            {action.icon}
          </div>
          <div>
            <div className="text-[13px] text-[#1E2939] mb-0.5">{action.label}</div>
...
function NewPredictionResultBody({ result }: ArtifactRendererProps) {
  ...
  // 构建"建议切入方向"数据 - 从 bestFor / primaryCard / secondaryCard 中提取
  ...
  const contentsByDirection = useMemo(() => {
    if (supportingContents.length === 0) return [[]];
    const chunkSize = Math.max(1, Math.ceil(supportingContents.length / Math.max(directions.length, 1)));
    return directions.map((_, i) =>
      supportingContents.slice(i * chunkSize, (i + 1) * chunkSize)
    );
  }, [supportingContents, directions]);

  const currentContents = contentsByDirection[selectedDirection] ?? contentsByDirection[0] ?? [];

  // 热门标签 - 从 supportingContents 的 keywordTokens 中提取
  const hotTags = useMemo(() => {
    const tagCount = new Map<string, number>();
    supportingContents.forEach(c => {
      ...
      <div className="bg-white rounded-[24px] border border-[#F3F4F6] shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <h2 className="text-[20px] text-[#101828] leading-[28px] mb-4 flex items-center gap-2">
                ...
                <span>{getVerdictLabel(result.verdict)}：{result.opportunityTitle || result.query}</span>
              </h2>
              <p className="text-[14px] text-[#1E2939] leading-[26px]">
                {result.summary}
              </p>
            </div>
            <div className="shrink-0"><CircularGauge value={result.score ?? 50} /></div>
          </div>
        </div>
        <div className="grid grid-cols-3 border-t border-[#F9FAFB]">
          <div className="px-6 py-4 border-r border-[#F9FAFB]">
            ...
            <div className="text-[14px] text-[#364153] mb-1">{getWindowStrengthLabel(result.windowStrength)}</div>
            <div className="text-[12px] text-[#99A1AF]">{market?.timingLabel ?? "—"}</div>
          </div>
          <div className="px-6 py-4 border-r border-[#F9FAFB]">
            <div className="flex items-center gap-1 text-[12px] text-[#6B38D4] mb-2"><BarChart3 className="w-3.5 h-3.5" /><span>竞争程度</span></div>
            <div className="text-[14px] text-[#364153] mb-1">{getMomentumLabel(market?.momentumLabel ?? "emerging")}</div>
            <div className="text-[12px] text-[#99A1AF]">{market?.evidenceWindowLabel ?? "—"}</div>
          </div>
          <div className="px-6 py-4">
            <div className="flex items-center gap-1 text-[12px] text-[#8979FF] mb-2"><Target className="w-3.5 h-3.5" /><span>你的差异空间</span></div>
            <div className="text-[14px] text-[#364153] mb-1">{result.confidenceLabel === "高" ? "较大" : result.confidenceLabel === "中" ? "中等" : "有限"}</div>
            <div className="text-[12px] text-[#99A1AF]">{result.accountMatchSummary || "—"}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 items-start">
        ...
        <div className="bg-white rounded-[24px] border border-[#F3F4F6] shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] px-5 py-6">
          ...
          <p className="text-[12px] text-[#6A7282] leading-[20px] mb-5">
            {whyNowItems.length > 0
              ? whyNowItems[0].inference
              : result.coreBet || "当前时机窗口正在打开，建议尽早行动。"}
          </p>
          {whyNowItems.length >= 3 ? (
            ...
            <div className="space-y-3">
              {whyNowItems.map((item, i) => (
                <div key={i} className="rounded-[12px] bg-[#F9FAFB] p-3">
                  <div className="text-[12px] text-[#8979FF] mb-1">{item.sourceLabel}</div>
                  <div className="text-[12px] text-[#364153]">{item.fact}</div>
                  <div className="text-[11px] text-[#99A1AF] mt-1">{item.userImpact}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex flex-col gap-4">
          ...
          {(result.notFor?.length ?? 0) > 0 && (
            <div className="bg-[#FFFBF0] rounded-[16px] border border-[#F7D5A6] px-4 py-3">
        ...
        <div className="bg-white rounded-[24px] border border-[#F3F4F6] shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] p-7">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-[#F9F8FF] rounded-full border border-[rgba(137,121,255,0.2)]">
              ...
              <span className="text-[11px] text-[#8979FF]">{directions[selectedDirection]?.title ?? "全部"}</span>
            </div>
          </div>
      ...
      <div className="bg-white rounded-[24px] border border-[#F3F4F6] shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] p-7">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-[#F9F8FF] rounded-full">
            <span className="text-[11px] text-[#8979FF]">基于：{directions[selectedDirection]?.title ?? result.query}</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[24px] border border-[#F3F4F6] shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] p-7">
          ...
          <div className="grid grid-cols-2 gap-8 items-start">
            ...
            <div className="bg-[#F9FAFB] rounded-[20px] p-6 flex flex-col gap-6">
              ...
              {/* 如果没有评论洞察，显示低粉爆款数据 */}
              {(!commentInsight || commentInsight.highlights.length === 0) && lowFollowerEvidence.length > 0 && (
                <div className="border-t border-[#EBEBEB] pt-5">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-[13px] text-[#1E2939]">低粉爆款数据</h3>
                    <span className="text-[11px] text-[#99A1AF]">{lowFollowerEvidence.length} 个样本</span>
                  </div>
                  <div className="space-y-2">
                    {lowFollowerEvidence.slice(0, 4).map((item, i) => (
                      <div key={i} className="rounded-[12px] bg-white border border-[#EBEBEB] p-3">
                        <div className="text-[12px] text-[#364153] mb-1">{item.title}</div>
                        <div className="flex items-center gap-2 text-[11px] text-[#99A1AF]">
                          <span>@{item.account}</span>
                          <span>{item.fansLabel}</span>
                          <span>{item.playCount}播放</span>
                          <span className="text-[#8979FF]">异常值 {item.anomaly}x</span>
                        </div>
                      </div>
                    ))}
                  </div>
