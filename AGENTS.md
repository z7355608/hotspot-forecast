...
> 给 AI 协作者(Codex / Cursor 等)和新加入的人类开发者的"驾驶舱仪表盘"。
...
| `pnpm test` | 跑全部 vitest 单测(目前 51 个 `.test.ts`) |
...
`.mcp.json` 里是 `${TIKHUB_API_KEY}`,**Codex 启动时从 shell 环境读**——
所以启动 `Codex` **之前**这个变量必须已经在当前 shell 里。最简单的方式:
